...
{
    if(reject && x[0] > 0.00 && x[0] < 0.06 && x[0] > 0.1 && x[0] < 0.18) { //upper limit 0.09 gives one kind f analysis method, while upper limit 0.11 gives anoher kind of analysis method
      TF1::RejectPoint();
      return 0;
   }
...
void WeibullFuncData(){
   ...
   //for(int i=0; i<8; i++){hBack->SetBinContent(i,0); hBack->SetBinError(i,1000);}
   //for(int i=11; i<20; i++){hBack->SetBinContent(i,0); hBack->SetBinError(i,1000);}
   hBack->Draw(); 
  
//for xF0 and xF1
   TF1 *fw = new TF1("fw",fReject,0.18,1,4); 
...
//for xF2 define the ranges of the function
   ...
   fw->Draw("same");
   reject=kTRUE; hBack->Fit(fw,"R"); reject=kFALSE;
