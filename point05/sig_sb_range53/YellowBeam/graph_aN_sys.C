...
*/
    ...
    double offset = 0.0065;
    double xF_val_offset_arr[bins]; //for shifting the yellow beam points manually on the x-axis
    for (int i = 0; i < bins; ++i) {
	xF_val_offset_arr[i] = xF_val_arr[i] + offset;
	}
    ...
    TGraphAsymmErrors *gr_stats = new TGraphAsymmErrors(bins, xF_val_offset_arr, aN_pi0_weibull_arr,
                                                        nullptr, nullptr, aN_pi0_err_weibull_arr, aN_pi0_err_weibull_arr);
    
    // Set style for the statistical points and error bars
    gr_stats->SetMarkerStyle(21); // square marker
    gr_stats->SetMarkerSize(1.4);
    gr_stats->SetMarkerColor(807); //kBlue
    gr_stats->SetLineColor(807); //kBlue
    ...
    TGraphAsymmErrors *gr_sys = new TGraphAsymmErrors(bins, xF_val_offset_arr, aN_pi0_weibull_arr,
                                                      x_err_sys_low, x_err_sys_high, optimum_pi0_err, optimum_pi0_err);
    ...
    mg->SetTitle("aN_{#pi^{0}} from Weibull Fit;x_{F};a_{N}"); // Set title and axis labels

    TFile *f = TFile::Open("PreliminaryPlots_YellowBeam.root","UPDATE");
    f->cd();
    mg->Write(Form("pi0_AN_%s_YB",method_input.c_str()));
