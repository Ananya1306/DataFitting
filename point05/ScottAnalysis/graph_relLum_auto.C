void graph_relLum_auto(string beam_input, string region_input){


	std::ofstream outfile;
       outfile.open(Form("aN_mod_relLum_%s_goodChi2.txt",region_input.c_str()),std::ios::out);
    ...
    // The main loop to process all xF bins
    for (int i = 0; i < xFBins; ++i) {
        int n = 0;
        fstream file;
        
        // Open the correct file for this iteration using a dynamic file path
        file.open(Form("auto_mod_relLum_%s_xF%d.txt", region_input.c_str(), i), std::ios::in);
        if (!file.is_open()) {
