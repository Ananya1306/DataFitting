...
//region_input = sig/bkg
//spin_input = YUp, YDown, BUp, BDown

//Here, instead of sig and sb, you have sig and bg. But, I am still going to keep the nomenclature same

	const int bins = 24; const int halfbins = 12; const double pi = TMath::Pi();
	double phiBUp, pi0sigErrBUp, pi0sbErrBUp; double pi0sigBUp, pi0sbBUp;
	double phiBUpArray[bins], pi0sigErrBUpArray[bins], pi0sbErrBUpArray[bins]; double pi0sigBUpArray[bins], pi0sbBUpArray[bins];

	double phiBDown, pi0sigErrBDown, pi0sbErrBDown; double pi0sigBDown, pi0sbBDown;
        double phiBDownArray[bins], pi0sigErrBDownArray[bins], pi0sbErrBDownArray[bins]; double pi0sigBDownArray[bins], pi0sbBDownArray[bins];
	...
	ifstream infile1;
        infile1.open(Form("sig_sb_range%d/%sBeam/%s_pi0_bkg_xF%d_range%d.txt",range_input,beam_input.c_str(),spin_input1.c_str(),xF_input,range_input),std::ios::in);
	...
	while(infile1 >> phiBUp >> pi0sigBUp >> pi0sigErrBUp >> pi0sbBUp >> pi0sbErrBUp ){
		...
		cout<<phiBUp <<" "<< pi0sigBUp <<" "<< pi0sigErrBUp <<" "<< pi0sbBUp <<" "<< pi0sbErrBUp<<endl;		
		i++;
	}
	...
	ifstream infile2;
        infile2.open(Form("sig_sb_range%d/%sBeam/%s_pi0_bkg_xF%d_range%d.txt",range_input,beam_input.c_str(),spin_input2.c_str(),xF_input,range_input),std::ios::in);
