...
void WeibullFuncEmbJP1(){
   ...
   for(int i=9; i<30; i++){hBack->SetBinContent(i,0); hBack->SetBinError(i,0.04);}
   hBack->Draw(); 
  
//for xF0 and xF1
   TF1 *fw = new TF1("fw",weibull_pdf,0.0,1,4); 
...
//for xF2 define the ranges of the function
   ...
   fw->SetParameters(0.8,0.9,0.03,0.01);
   fw->Draw("same");
   hBack->Fit(fw,"R");
