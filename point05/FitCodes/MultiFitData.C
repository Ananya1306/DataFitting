...
void MultiFitData(){
   ...
   //for(int i=9; i<25; i++){hBack->SetBinContent(i,0); hBack->SetBinError(i,2000);}
   hBack->Draw("lpe");

   TF1 *fsg = new TF1("fsg",SkGausPeak,0.0,0.08,4);
   fsg->SetParameters(1600,0.05,0.01,4); //Scale, Mean, Sigma, Skewness
   ...
   TF1 *fw = new TF1("fw",weibull_pdf,0.3,1,4);
   ...
   TF1 *ftot = new TF1("ftot",fRejBack,0,1,8);
   ftot->SetParameters(Shape,WeibullScale,Location,Amplitude,GausScale, Mean,Sigma,Skewness);
   ftot->SetLineColor(kRed); reject = kTRUE; hBack->Fit(ftot,"R+"); reject = kFALSE;
}
