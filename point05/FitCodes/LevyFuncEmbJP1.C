...
void LevyFuncEmbJP1(){
   ...
   TFile *f = new TFile("../EEmcNanoTreeEmb_JP1Trig_All.root");
   TH1F *h = (TH1F*)f->Get("pi0MxFPhi_0");
   TH1F *hBack = (TH1F*)h->Clone("hBack");
   TH1F *hBackFit = (TH1F*)h->Clone("hBackFit");
   for(int i=11; i<18; i++){hBack->SetBinContent(i,0); hBack->SetBinError(i,100);}
   hBack->Draw(); 
   ...
   fl->SetParameters(0.05,0.8,0.01); //pi0MxFPhi_0 fit initial parameters
   fl->Draw("same");
   hBack->Fit(fl,"R");
