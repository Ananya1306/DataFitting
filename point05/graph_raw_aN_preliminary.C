void graph_raw_aN_preliminary(int range_input = 53, string beam_input = "Blue", string method_input = "crossRatio"){
...
//method_input = crossRatio/relLum
	...
	cout<<"Blue Beam sideband region"<<endl;
        ...
        fstream file1;
	file1.open(Form("sig_sb_range%d/%sBeam/aN_mod_indv_%s_sb_goodChi2.txt",range_input,beam_input.c_str(),method_input.c_str()),std::ios::in);
	...
	f->cd();
 	mg->Write(Form("graph_raw_aN_indvFit_%sBeam_%s",beam_input.c_str(),method_input.c_str()));
