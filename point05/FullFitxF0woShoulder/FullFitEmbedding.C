...
{
    if (reject && x[0] > 0.00 && x[0] < 0.1) { //upper limit 0.09 gives one kind f analysis method, while upper limit 0.11 gives anoher kind of analysis method
      TF1::RejectPoint();
      return 0;
   }
...
//For xF0 fir embedding
   ...
   fitFcn->SetParameter(0,1.09285e+00); //weibull shape
   fitFcn->SetParameter(1,4.41395e-01); //weibull scale
   fitFcn->SetParameter(2,6.50109e-02);  //weibull location
   fitFcn->SetParameter(3,1.91154e+02); //weibull amplitude
   fitFcn->SetParameter(4,1.19376e+03); //skgaus amplitude
   fitFcn->SetParameter(5,1.45801e-01); //skgaus mean
   fitFcn->SetParameter(6,5.06436e-02); //skgaus sigma
   fitFcn->SetParameter(7,-1.71016e-01); //skgaus skewness   
...
*/   
//For xF2 fit embedding
/*    
   fitFcn->SetParameter(0,1.44158e+00); //weibull shape
   fitFcn->SetParameter(1,6.07249e-01); //weibull scale
   fitFcn->SetParameter(2,6.44603e-02);  //weibull location
   fitFcn->SetParameter(3,2.46689e+02); //weibull amplitude
   fitFcn->SetParameter(4,1.52321e+03); //skgaus amplitude
   fitFcn->SetParameter(5,1.51929e-01); //skgaus mean
   fitFcn->SetParameter(6,4.41792e-02); //skgaus sigma
   fitFcn->SetParameter(7,-1.74061e-01); //skgaus skewness   
