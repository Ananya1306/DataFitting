...
void FullFit_Data_BBeam_modRange_xF5bins_LogNormal(int range_input, int xF_input, string beam_input){

   std::ofstream outfile;
   outfile.open(Form("../sig_sb_range%d/%sBeam/fSig_LogNormal.txt",range_input,beam_input.c_str()),std::ios::app);
...
*/
     ...
     TH1F *h = (TH1F*)f->Get(Form("pi0M_BAll_xF%d_phi0",xF_input)); h->GetXaxis()->SetTitle("M_{#gamma#gamma} GeV/c^{2}");
...
//For xF4 Data
   ...
   fitFcnDraw->SetRange(sb_Low_1, sb_High_1);
   fitFcnDraw->SetFillColor(kRed); 
   fitFcnDraw->SetFillStyle(3001);
   fitFcnDraw->Draw("SAME FC R");

	cout<<"FitFcn Integral in the lower mass region = "<<fitFcn->Integral(sb_Low_1,sb_High_1)<<endl;
/*
